...
class Settings(BaseSettings):
    ...
    # Subset of admin emails that also use the performance dashboard (not admin-only)
    sso_static_dashboard_emails: str = "shivanshu.choudhary@sobharealty.com"
...
def static_sso_dashboard_emails() -> set[str]:
    return {e.strip().lower() for e in settings.sso_static_dashboard_emails.split(",") if e.strip()}


def is_static_sso_dashboard_admin(email: Optional[str]) -> bool:
    if not email:
        return False
    return email.strip().lower() in static_sso_dashboard_emails()
