...
try:
    ...
    from .config import settings, is_static_sso_admin, is_static_sso_dashboard_admin
except ImportError:
    ...
    from backend.config import settings, is_static_sso_admin, is_static_sso_dashboard_admin
...
def require_admin(payload: dict = Depends(auth_header), db: Session = Depends(get_db)) -> dict:
    """Require admin role or a configured static admin Microsoft email."""
...
def _sso_success_html(token: str, user_payload: dict, state: Optional[str]) -> HTMLResponse:
    ...
    if flow == "admin" and role != "admin":
        ...
        if not is_static_sso_admin(email):
            msg = "This Microsoft account is not authorized for admin access."
            return _sso_error_html(msg, "/admin")

    # Keep flows separate: dashboard SSO always returns to the user app;
    # admin SSO returns to /admin. Do not send dashboard logins to the admin page.
    redirect_to = "/admin" if flow == "admin" else "/"
...
def apply_static_sso_admin(
    user: UserProfile,
    microsoft_user: MicrosoftUserInfo,
    db: Session,
) -> UserProfile:
    """Promote configured Microsoft emails to admin; dual-access emails also get dashboard."""
    email = (microsoft_user.email or user.email or "").strip().lower()
    if not is_static_sso_admin(email):
        return user
    ...
    if microsoft_user.display_name:
        user.name = microsoft_user.display_name

    if is_static_sso_dashboard_admin(email):
        # Full performance dashboard (Management + all data) plus admin panel via email allowlist.
        user.role = "Management"
        user.scope_type = "all"
        user.scope_value = "all"
    else:
        user.role = "admin"
        user.scope_type = user.scope_type or "management"
        user.scope_value = user.scope_value or "all"
...
def get_or_create_user_from_sso(
    ...
    if auto_create:
        ...
        user = UserProfile(
            ...
            role=(
                "Management"
                if is_static_sso_dashboard_admin(microsoft_user.email)
                else "admin"
                if is_static_sso_admin(microsoft_user.email)
                else settings.sso_default_role
            ),
            scope_type=(
                "all"
                if is_static_sso_dashboard_admin(microsoft_user.email)
                ...
                if is_static_sso_admin(microsoft_user.email)
                else settings.sso_default_scope_type
            ),
            scope_value=(
                "all"
                if is_static_sso_dashboard_admin(microsoft_user.email)
                or is_static_sso_admin(microsoft_user.email)
                else settings.sso_default_scope_value
            ),
...
async def microsoft_callback_get(
    ...
    try:
        ...
        user = apply_static_sso_admin(user, microsoft_user, db)
...
async def microsoft_callback_post(
    ...
    try:
        ...
        if flow == "admin" and user.role != "admin":
            if not is_static_sso_admin(user.email):
                raise HTTPException(
                    status_code=403,
