...
# Admin panel API under /admin/* (not /api/*) — avoids Cloudflare WAF blocks on POST /api/v1/*
admin_panel_router = APIRouter(prefix="/admin", tags=["admin-panel"])


@admin_panel_router.post("/login")
def admin_panel_login(body: LoginRequest, db: Session = Depends(get_db)):
    return authenticate_login(body, db)
...
if FRONTEND_DIR.exists():
    ...
    @app.get("/dashboard")
    def dashboard():
        # Serve index.html - frontend will load the dashboard if authenticated
        return FileResponse(FRONTEND_DIR / "index.html")
